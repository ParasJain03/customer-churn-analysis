...
def gemini_call(prompt):
    ...
    if not key or genai is None:
        raise RuntimeError("Gemini is not connected. Check GEMINI_API_KEY in .env")
...
with right:
    profile = pd.DataFrame([(c.replace('_',' '), str(customer[c])) for c in customer.index if c != 'Customer_ID'], columns=["Attribute","Value"])
...
question = st.text_input("Ask Your Data", placeholder="Examples: What is total predicted churn? | Which state has highest churn rate? | How many female churners? | Average monthly charge? | Top 5 contracts | Tell me about customer 62359-AND", key="ai_question")
ask = st.button("🔍 Analyze", type="primary")

# Gemini only creates an intent plan. It never supplies dashboard numbers.
...
def fallback_plan(question, df):
    q = question.lower()
    aliases = {"gender":"Gender","state":"State","contract":"Contract","internet":"Internet_Type","risk":"Risk_Level","priority":"Retention_Priority","payment":"Payment_Method","married":"Married","tenure":"Tenure_in_Months","monthly charge":"Monthly_Charge","monthly":"Monthly_Charge","revenue":"Total_Revenue","refund":"Total_Refunds","referral":"Number_of_Referrals","probability":"Churn_Probability"}
    group = next((v for k,v in sorted(aliases.items(), key=lambda x:-len(x[0])) if k in q and v in df.columns and not k in ['monthly','revenue','refund','referral','probability']), None)
    ...
    if any(w in q for w in ["average","mean"]): metric="average"
    elif any(w in q for w in ["highest","maximum","max","lowest","minimum","min"]): metric="max" if any(w in q for w in ["highest","maximum","max"]) else "min"
    elif "rate" in q or "percentage" in q: metric="predicted_churn_rate"
    elif "churn" in q: metric="predicted_churners"
    else: metric="customers"
...
def execute(plan, df):
    typ=plan.get("type","overview"); group=plan.get("group_by"); target=plan.get("target_column"); metric=plan.get("metric","customers"); n=max(1,min(50,int(plan.get("top_n",10))))
    if typ == "customer":
        row=find_customer(question,df)
        return {"kind":"customer","row":row}
    if group not in df.columns: group=None
    if group:
        t=segment_table(df,group)
        value={"customers":"Customers","predicted_churners":"Predicted_Churners","predicted_churn_rate":"Predicted_Churn_Rate","average_churn_probability":"Avg_Churn_Probability"}.get(metric,"Predicted_Churn_Rate")
    ...
    if metric in ["average","min","max"]:
        ...
        if not target: return {"kind":"empty"}
        v=pd.to_numeric(df[target],errors="coerce").dropna(); val=v.mean() if metric=="average" else v.min() if metric=="min" else v.max()
        return {"kind":"numeric","target":target,"metric":metric,"value":val,"count":len(v)}
    m=prediction_metrics(df)
    return {"kind":"overview","metrics":m}
...
if ask and question.strip():
    try:
        ...
        if result["kind"]=="overview":
            ...
            t=result["table"]; group=result["group"]; value=result["value"]
            top=t.iloc[0]
            ...
            if value=="Predicted_Churn_Rate": q3.metric("Top Segment Churn Rate",f"{float(top[value]):.1%}")
            else: q3.metric("Top Segment",str(top[group]))
            ...
            st.dataframe(pd.DataFrame([{"Column":result['target'],"Records":result['count'],"Average":pd.to_numeric(filtered[result['target']],errors='coerce').mean(),"Minimum":pd.to_numeric(filtered[result['target']],errors='coerce').min(),"Maximum":pd.to_numeric(filtered[result['target']],errors='coerce').max()}]),width="stretch",hide_index=True)
        elif result["kind"]=="customer":
            row=result["row"]
            if row is None: st.warning("Customer was not found in the current filtered data.")
            else:
                q1,q2,q3,q4=st.columns(4)
                ...
                st.dataframe(pd.DataFrame([(c.replace('_',' '),str(row[c])) for c in row.index if c!='Customer_ID'],columns=['Column','Value']),width='stretch',hide_index=True)
        ...
        # Gemini explains the already-calculated result in simple English.
        explanation_data={k:v for k,v in result.items() if k not in ['row']}
        if result["kind"]=="group": explanation_data["rows"]=result["table"].head(10).to_dict(orient='records')
        explanation_prompt=f"""Explain this telecom BI result in simple English. Never change or invent numbers. Give one direct answer and up to 3 short insights. Do not claim causation. Clearly say predicted churn, not confirmed churn. User question: {question}\nCalculated result: {json.dumps(explanation_data,default=str)}"""
        try:
...
if rv.empty: rv=filtered.copy()
cols=[c for c in ["Customer_ID","Churn_Probability_Percent","Risk_Level","Retention_Priority","Contract","Tenure_in_Months","Monthly_Charge","Retention_Recommendation"] if c in rv.columns]
